...
unsafe fn ClearKeyTypesInfo(mut info: *mut KeyTypesInfo) {
    unsafe {
        ...
        if !(*info).types.item.is_null() {
            type_0 =
                (*info).types.item.offset(0 as i32 as isize) as *mut KeyTypeInfo;
            while type_0
    ...
    unsafe {
        ...
        if (*into).types.size == 0 as darray_size_t {
            ...
            if !(*from).types.item.is_null() {
                type_0 =
                    (*from).types.item.offset(0 as i32 as isize) as *mut KeyTypeInfo;
                while type_0
...
unsafe fn HandleGlobalVar(mut info: *mut KeyTypesInfo, mut stmt: *mut VarDef) -> bool {
    unsafe {
        ...
        if !ExprResolveLhs(
            ...
            && istreq(elem, b"type\0".as_ptr() as *const i8) as i32 != 0
        {
            xkb_logf!(
                (*info).ctx,
                XKB_LOG_LEVEL_ERROR,
