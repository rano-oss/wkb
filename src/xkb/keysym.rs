...
pub(crate) fn xkb_keysym_to_upper(ks: u32) -> u32 {
    ...
    if let Some(ch) = char::from_u32(ks.wrapping_sub(XKB_KEYSYM_UNICODE_OFFSET)) {
        if let Some(upper) = ch.to_uppercase().next() {
            return XKB_KEYSYM_UNICODE_OFFSET + upper as u32;
        }
    }
...
pub(crate) fn xkb_keysym_is_lower(ks: u32) -> bool {
    char::from_u32(ks.wrapping_sub(XKB_KEYSYM_UNICODE_OFFSET))
