...
pub(crate) static KEYSYM_TO_NAME: [(u32, u32); 2502] = [
    (0, 20276),
    (0x20, 24027),
    (0x21, 12230),
    (0x22, 22385),
...
fn find_keysym_index(ks: u32) -> isize {
    ...
    KEYSYM_TO_NAME
        .binary_search_by(|entry| entry.0.cmp(&ks))
        .map(|i| i as isize)
...
pub(crate) fn keysym_get_name(ks: u32) -> Option<&'static str> {
    ...
    if index != -1_i32 as isize {
        let name_bytes = get_name_bytes(KEYSYM_TO_NAME[index as usize].1);
        ...
        while hi >= lo {
            ...
            if cmp > 0 {
                ...
                for (candidate, entry) in NAME_TO_KEYSYM.iter().enumerate().skip(mid + 1) {
                    if istrcmp(get_name_bytes(entry.1), get_name_bytes(NAME_TO_KEYSYM[idx].1)) == 0 {
                        idx = candidate;
                    } else {
                        break;
                    }
...
fn bin_search_keysym(keysym: u32) -> Option<u32> {
    ...
    let idx = KEYSYM_TABLE
        .binary_search_by_key(&keysym16, |entry| entry.0)
        .ok()?;
    Some(KEYSYM_TABLE[idx].1 as u32)
}
...
pub(crate) fn xkb_keysym_to_upper(mut ks: u32) -> u32 {
    if ks <= 0x13be_u32 {
        let (_lower, upper, offset) = get_legacy_keysym_entry(ks);
        if upper as i32 != 0 {
            ks.wrapping_sub(offset as u32)
        } else {
            ks
        }
    } else if (XKB_KEYSYM_UNICODE_MIN..=0x101f189_u32).contains(&ks) {
        let (_lower, upper, offset) = get_unicode_entry(ks.wrapping_sub(XKB_KEYSYM_UNICODE_OFFSET));
        if upper {
...
pub(crate) fn xkb_keysym_is_lower(ks: u32) -> bool {
    if ks <= 0x13be_u32 {
        let (lower, upper, _offset) = get_legacy_keysym_entry(ks);
        upper as i32 != 0 && !lower
    } else if (XKB_KEYSYM_UNICODE_MIN..=0x101f189_u32).contains(&ks) {
        let (lower, upper, _offset) = get_unicode_entry(ks.wrapping_sub(XKB_KEYSYM_UNICODE_OFFSET));
        upper as i32 != 0 && !lower
    } else {
        false
    }
...
pub(crate) fn xkb_keysym_is_upper_or_title(ks: u32) -> bool {
    if ks <= 0x13be_u32 {
        let (lower, _upper, _offset) = get_legacy_keysym_entry(ks);
        lower
    } else if (XKB_KEYSYM_UNICODE_MIN..=0x101f189_u32).contains(&ks) {
        let (lower, _upper, _offset) = get_unicode_entry(ks.wrapping_sub(XKB_KEYSYM_UNICODE_OFFSET));
        lower
    } else {
        false
    }
