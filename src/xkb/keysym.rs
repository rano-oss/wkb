...
pub(crate) static NAME_TO_KEYSYM: [NameKeysym; 2635] = [
    NameKeysym {
        keysym: 0x30,
        offset: 0,
    },
...
pub(crate) static KEYSYM_TO_NAME: [NameKeysym; 2502] = [
    NameKeysym {
        keysym: 0,
        offset: 20276,
    },
...
fn find_keysym_index(ks: u32) -> isize {
    ...
    KEYSYM_TO_NAME
        .binary_search_by(|entry| entry.keysym.cmp(&ks))
        .map(|i| i as isize)
        .unwrap_or(-1_i32 as isize)
}
#[inline]
fn get_name_bytes(entry: &NameKeysym) -> &'static [u8] {
...
pub(crate) fn xkb_keysym_from_name(name: &[u8], flags: u32) -> Option<u32> {
    ...
    if !icase {
        ...
        if pos < NAME_TO_KEYSYM.len() && name_bytes == get_name_bytes(&NAME_TO_KEYSYM[pos]) {
            return Some(NAME_TO_KEYSYM[pos].keysym);
        }
    } else {
        let mut lo: i32 = 0;
