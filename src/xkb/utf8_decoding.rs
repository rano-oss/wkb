    ...
    unsafe {
        ...
        *size_out = 0 as size_t;
        if max_size == 0 || len as size_t > max_size {
            return INVALID_UTF8_CODE_POINT as uint32_t;
        }
        match len as ::core::ffi::c_int {
            1 => {
                *size_out = 1 as size_t;
                return *s.offset(0 as ::core::ffi::c_int as isize) as uint32_t;
            }
            2 => {
                cp = *s.offset(0 as ::core::ffi::c_int as isize) as uint32_t & 0x1f as uint32_t;
            }
            3 => {
                cp = *s.offset(0 as ::core::ffi::c_int as isize) as uint32_t & 0xf as uint32_t;
            }
            4 => {
                cp = *s.offset(0 as ::core::ffi::c_int as isize) as uint32_t & 0x7 as uint32_t;
            }
            _ => return INVALID_UTF8_CODE_POINT as uint32_t,
        }
        let mut k: size_t = 1 as size_t;
        while k < len as size_t {
            if *s.offset(k as isize) as uint32_t & 0xc0 as uint32_t != 0x80 as uint32_t {
                return INVALID_UTF8_CODE_POINT as uint32_t;
            }
            cp <<= 6 as ::core::ffi::c_int;
            cp |= *s.offset(k as isize) as uint32_t & 0x3f as uint32_t;
            k = k.wrapping_add(1);
        }
        if is_surrogate(cp) {
            return INVALID_UTF8_CODE_POINT as uint32_t;
