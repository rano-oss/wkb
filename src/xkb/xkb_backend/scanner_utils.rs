...
impl<'a> scanner<'a> {
    ...
    pub fn buf_appends_code_point(&mut self, c: u32) -> bool {
        if self.buf_pos + 4 <= self.buf.len() {
            let mut count = super::xkbcomp::scanner::utf8_h::utf32_to_utf8_safe(
                c,
                &mut self.buf[self.buf_pos..],
            );
            if count == 0 {
                count = super::xkbcomp::scanner::utf8_h::utf32_to_utf8_safe(
                    0xfffd,
                    &mut self.buf[self.buf_pos..],
                );
            }
    ...
    pub fn dec_int64(&mut self, out: &mut i64) -> i32 {
        ...
        let (val, count) = super::utils::parse_dec_u64(remaining);
        if count > 0 {
    ...
    pub fn hex_int64(&mut self, out: &mut i64) -> i32 {
        let remaining = self.remaining_bytes();
        let (val, count) = super::utils::parse_hex_u64(remaining);
        if count > 0 {
            if val > i64::MAX as u64 {
                return -1;
            }
    ...
    pub fn unicode_code_point(&mut self, out: &mut u32) -> bool {
        ...
        let remaining = self.remaining_bytes();
        let (cp, count) = super::utils::parse_hex_u32(remaining);
        if count > 0 {
            self.pos += count as usize;
        }
