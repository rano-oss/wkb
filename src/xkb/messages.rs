...
use self::test_h::test_init;
unsafe extern "C" fn test_message_get() {
    unsafe {
        let mut entry: *const xkb_message_entry = ::core::ptr::null::<xkb_message_entry>();
        entry = xkb_message_get(0 as xkb_message_code);
        if entry.is_null() {
        } else {
            __assert_fail(
                b"entry == NULL\0".as_ptr() as *const ::core::ffi::c_char,
                b"../test/messages.c\0".as_ptr() as *const ::core::ffi::c_char,
                23 as ::core::ffi::c_uint,
                b"void test_message_get(void)\0".as_ptr() as *const ::core::ffi::c_char,
            );
        };
        entry = xkb_message_get(
            (_XKB_LOG_MESSAGE_MIN_CODE as ::core::ffi::c_int - 1 as ::core::ffi::c_int)
                as xkb_message_code,
        );
        if entry.is_null() {
        } else {
        ...
        entry = xkb_message_get(
            (_XKB_LOG_MESSAGE_MAX_CODE as ::core::ffi::c_int + 1 as ::core::ffi::c_int)
                as xkb_message_code,
        );
        if entry.is_null() {
        } else {
