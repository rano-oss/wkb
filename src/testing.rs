...
pub mod compose_parse {
    ...
    pub use crate::xkb::{load_compose_from_path, load_compose_from_path_uncached};
}
...
/// All `(layout, variant)` pairs from the XKB registry. The base layout has
/// an empty variant. Layouts/locales are static, so this list is baked in
/// instead of parsing `rules/*.xml` registry files at runtime.
