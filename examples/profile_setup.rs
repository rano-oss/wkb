...
fn main() {
    ...
    let _wkb: wkb::WKB<wkb::testing::ListComposer> =
        wkb::WKB::new_from_names("us".to_string(), None);
    eprintln!("\nFull WKB new_from_names: {:?}", t_wkb.elapsed());

    // With explicit layout (skip get_all_layouts)
    let t_wkb2 = Instant::now();
    let _wkb2: wkb::WKB<wkb::testing::ListComposer> =
        wkb::WKB::new_from_names("us".to_string(), Some(String::new()));
