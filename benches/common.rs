...
use wkb::testing::{Composer, Token};

// ── Precompiled KDL layout loading (the "no XKB" construction path) ─────
/// Directory holding precompiled layout files (generated by `gen_layouts`).
pub const KDL_LAYOUTS_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/kdl_layouts");

/// Path to the precompiled KDL layout for `(locale, variant)`, e.g. `us.kdl`
/// or `us.intl.kdl`.
pub fn kdl_layout_path(locale: &str, variant: Option<&str>) -> String {
    match variant {
        Some(v) => format!("{KDL_LAYOUTS_DIR}/{locale}.{v}.kdl"),
        None => format!("{KDL_LAYOUTS_DIR}/{locale}.kdl"),
    }
}

/// Load one precompiled layout file.
pub fn load_layout_file(locale: &str, variant: Option<&str>) -> wkb::ir::LayoutFile {
    let path = kdl_layout_path(locale, variant);
    let text = std::fs::read_to_string(&path).expect("read kdl layout file");
    wkb::ir::LayoutFile::from_kdl_str(&text).expect("parse kdl layout file")
}

/// Ensure the precompiled KDL layout for `(locale, variant)` exists,
/// generating it from the XKB registry if missing.
///
/// `kdl_layouts/` is a gitignored generated-artifact directory, so a fresh
...
pub fn ensure_layout_file(locale: &str, variant: Option<&str>) {
    ...
    std::fs::create_dir_all(KDL_LAYOUTS_DIR).expect("create kdl layout dir");
    ...
    let kdl = file.to_kdl_string().expect("serialize layout fixture");
    std::fs::write(&path, kdl).expect("write kdl layout fixture");
}
...
/// compose benches: the KDL carries a compose table, so the no-XKB path can
...
pub fn composer_from_layout_file(file: &wkb::ir::LayoutFile) -> Composer {
    ...
    for (keys, output) in &file.compose {
        let tokens: Vec<Token> = keys
            .iter()
            .map(|&ch| {
                if ch == wkb::ir::COMPOSE_KEY_CHAR {
                    Token::Compose
                } else {
                    Token::Char(ch)
                }
