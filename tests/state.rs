...
fn level3(locale: &str) {
    ...
    for layout in wkb.layouts() {
        ...
        // level 3
        if wkb.level_keymap.len() < 3 || wkb.modifiers.level3shift.0 .0 == 0 {
            continue;
        }
        if wkb.modifiers.level3shift.0 .0 == 0 {
            println!("{}", layout);
        }
...
fn level4(locale: &str) {
    ...
    for layout in wkb.layouts() {
        ...
        let mut wkb = wkb::WKB::new_from_names(locale.to_string(), Some(layout.to_owned()));
        // level 4
        if wkb.level_keymap.len() < 4 || wkb.modifiers.level3shift.0 .0 == 0 {
            continue;
        }
        let i = wkb.modifiers.level3shift.0 .0;
        xkb.update_key(Keycode::new(i as u32 + 8), xkb::KeyDirection::Down);
...
fn level5(locale: &str) {
    ...
    for layout in wkb.layouts() {
        ...
        let mut wkb = wkb::WKB::new_from_names(locale.to_string(), Some(layout.to_owned()));
        // level 5
        if wkb.level_keymap.len() < 5 || wkb.modifiers.level5shift.0 == 0 {
            continue;
        }
        let i = wkb.modifiers.level5shift.0;
        // assert!(i != 0);
        xkb.update_key(Keycode::new(i as u32 + 8), xkb::KeyDirection::Down);
        wkb.update_key(i, wkb::KeyDirection::Down);
        test_all_keys(wkb, xkb, layout);
...
fn level6(locale: &str) {
    ...
    for layout in wkb.layouts() {
        ...
        let mut xkb = xkb_new_from_names(locale.to_string(), Some(layout.to_owned()));
        let mut wkb = wkb::WKB::new_from_names(locale.to_string(), Some(layout.to_owned()));
        if wkb.level_keymap.len() < 6 || wkb.modifiers.level5shift.0 == 0 {
            continue;
        }
        // level 5
        let i = wkb.modifiers.level5shift.0;
...
fn level7(locale: &str) {
    ...
    for layout in wkb.layouts() {
        let mut xkb = xkb_new_from_names(locale.to_string(), Some(layout.to_owned()));
        let mut wkb = wkb::WKB::new_from_names(locale.to_string(), Some(layout.to_owned()));
        if wkb.level_keymap.len() < 7 || wkb.modifiers.level5shift.0 == 0 {
            continue;
        }
        // level 5
        let i = wkb.modifiers.level5shift.0;
...
fn level8(locale: &str) {
    ...
    for layout in wkb.layouts() {
        ...
        let mut xkb = xkb_new_from_names(locale.to_string(), Some(layout.to_owned()));
        let mut wkb = wkb::WKB::new_from_names(locale.to_string(), Some(layout.to_owned()));
        if wkb.level_keymap.len() < 8 || wkb.modifiers.level5shift.0 == 0 {
            continue;
        }
        // level 5
        let i = wkb.modifiers.level5shift.0;
